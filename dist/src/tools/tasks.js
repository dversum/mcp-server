...
export function registerTaskTools(server, client) {
    ...
    server.tool("delete_task", "Delete a task. This cannot be undone.", {
        id: z.string().describe("The task UUID"),
    }, async ({ id }) => {
    ...
    server.tool("create_subtask", "Create a subtask (checklist item) on a task.", {
    ...
    server.tool("toggle_task_done", "Toggle a task's completion status.", {
        id: z.string().describe("The task UUID"),
    }, async ({ id }) => {
    ...
    server.tool("remove_assignee", "Remove an assignee from a task.", {
        task_id: z.string().describe("The task UUID"),
    ...
    server.tool("add_tag_to_task", "Add a tag to a task.", {
        task_id: z.string().describe("The task UUID"),
    ...
    server.tool("remove_tag_from_task", "Remove a tag from a task.", {
        task_id: z.string().describe("The task UUID"),
        tag_id: z.string().describe("The tag UUID"),
    }, async ({ task_id, tag_id }) => {
    ...
    server.tool("archive_task", "Archive a task.", {
        id: z.string().describe("The task UUID"),
    }, async ({ id }) => {
    ...
    server.tool("restore_task", "Restore an archived task.", {
        id: z.string().describe("The task UUID"),
    }, async ({ id }) => {
    ...
    server.tool("move_task_to_project", "Move a task to a different project.", {
        id: z.string().describe("The task UUID"),
    ...
    server.tool("create_comment", "Add a comment to a task.", {
        task_id: z.string().describe("The task UUID"),
