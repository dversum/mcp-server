import { z } from "zod";
export function registerAreaTools(server, client) {
    server.tool("list_areas", "List all project areas/groups.", {}, async () => {
        const data = await client.get("/areas");
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("create_area", "Create a new project area/group.", {
    ...
    server.tool("update_area", "Update a project area/group.", {
    ...
    server.tool("delete_area", "Delete a project area/group.", {
        id: z.string().describe("The area UUID"),
    }, async ({ id }) => {
