...
export function registerOrganizationTools(server, client) {
    ...
    server.tool("invite_member", "Invite a new member to the organization via email.", {
    ...
    server.tool("update_member_role", "Update a member's role in the organization.", {
        org_id: z.string().describe("The organization UUID"),
    ...
    server.tool("remove_member", "Remove a member from the organization.", {
        org_id: z.string().describe("The organization UUID"),
    ...
    server.tool("global_search", "Search across all entities (clients, projects, tasks, invoices, etc.).", {
    ...
    server.tool("get_activity_feed", "Get the recent activity feed.", {}, async () => {
