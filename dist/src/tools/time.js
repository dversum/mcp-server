...
export function registerTimeTools(server, client) {
    server.tool("list_time_entries", "List time entries. Filter by date range, project, or user.", {
    ...
    server.tool("create_time_entry", "Log a completed time entry with start and end time.", {
    ...
    server.tool("start_timer", "Start a running timer for time tracking.", {
        project_id: z.string().describe("Project UUID"),
        task_id: z.string().optional().describe("Task UUID (optional)"),
    ...
    server.tool("stop_timer", "Stop a currently running timer.", {
    ...
    server.tool("get_running_timer", "Get the currently running timer, if any.", {}, async () => {
        const data = await client.get("/time-entries/running");
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("get_time_stats", "Get time tracking statistics (total hours, billable hours, etc.).", {
        project_id: z.string().optional().describe("Filter by project UUID"),
