import { z } from "zod";
export function registerWhiteboardTools(server, client) {
    server.tool("list_whiteboards", "List all whiteboards.", {}, async () => {
        const data = await client.get("/whiteboards");
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
    });
    server.tool("get_whiteboard", "Get whiteboard details including content.", {
    ...
    server.tool("create_whiteboard", "Create a new whiteboard.", {
    ...
    server.tool("update_whiteboard", "Update whiteboard metadata.", {
        id: z.string().describe("The whiteboard UUID"),
    ...
    server.tool("delete_whiteboard", "Delete a whiteboard.", {
        id: z.string().describe("The whiteboard UUID"),
    }, async ({ id }) => {
    ...
    server.tool("duplicate_whiteboard", "Duplicate an existing whiteboard.", {
