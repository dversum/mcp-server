import { z } from "zod";
export function registerReminderTools(server, client) {
    server.tool("list_reminders", "List payment reminders (Mahnungen).", {
    ...
    server.tool("get_reminder", "Get details of a payment reminder.", {
    ...
    server.tool("get_reminder_suggestion", "Get a suggested reminder for an overdue invoice (auto-detects level and fees).", {
    ...
    server.tool("create_reminder", "Create a payment reminder (Mahnung) for an overdue invoice.", {
    ...
    server.tool("send_reminder_email", "Send a payment reminder via email.", {
        id: z.string().describe("The reminder UUID"),
    ...
    server.tool("delete_reminder", "Delete a payment reminder.", {
        id: z.string().describe("The reminder UUID"),
    }, async ({ id }) => {
